...
export const CarouselTvMovies = () => {
  ...
  useEffect(() => {
    const getTvMoviesMovies = async () => {
      try {
        const response = await axios.get(
          baseURL +
            '/discover/movie?include_adult=false&include_video=true&language=en-US&page=1&sort_by=vote_count.desc&with_genres=10770',
          {
            headers: {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>TV Movie</Text>
      <FlatList
